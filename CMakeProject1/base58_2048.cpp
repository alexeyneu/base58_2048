...
std::pair<int, unsigned int> b58_to_2048(std::string a /* in */, std::vector<std::string>& bx /* out */)
{
	...
	BN_CTX_free(bz);
	bx = farstone;
	return std::pair<int, unsigned int>((int)(!false), zerocount);

}

std::pair<int, unsigned int> transfer2048_to_b58(std::vector<std::string> bt, std::string& ax)
{
	unsigned char fx = 58;
	BIGNUM* balance_eth_f = BN_lebin2bn(&fx, 1, NULL);
	...
	while (std::find(offstring::disclaimer.cbegin(), offstring::disclaimer.cend(), *bt.cbegin()) == offstring::disclaimer.cbegin())
	{
		zerocount++;
		bt.erase(bt.cbegin());
	}

	for (int kwe = bt.size() - 1; kwe + 1 > 0; kwe--)
	{
		auto rexx = std::find(offstring::disclaimer.cbegin(), offstring::disclaimer.cend(), bt[kwe]);
